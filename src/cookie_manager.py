# -*- coding: utf-8 -*-
# WeRead Cookie 管理模块，支持 CookieCloud 获取、AES 解密、自动验证和刷新
...
class CookieUtil:
    ...
    def parse_cookie_string(cookie_string: str) -> Dict[str, str]:
        ...
        :param cookie_string: Cookie 字符串
        :return: Cookie 字典
        """
    ...
    def parse_cookies_to_list(cookie_string: str) -> List[Cookie]:
        ...
        示例:
            "wr_name=user; wr_vid=123" -> [Cookie(name="wr_name", value="user"), Cookie(name="wr_vid", value="123")]
        :param cookie_string: Cookie 字符串
        :return: Cookie 列表
        """
...
# CookieCloud Cookie 解密器，支持 CryptoJS/OpenSSL 兼容的 AES 解密（EVP_BytesToKey 密钥派生）
# 参考实现:
...
class CookieCloudDecryptor:
    ...
    def _evp_bytes_to_key(password: bytes, salt: bytes, key_len: int = 32, iv_len: int = 16, 
        ...
        """
        实现 OpenSSL EVP_BytesToKey 密钥派生算法，这是 CryptoJS AES 加密使用的标准密钥派生方式
        
        :param password: 密码字节
        :param salt: 盐值字节（8字节）
        :param key_len: 派生密钥长度（默认32字节用于AES-256）
        :param iv_len: 派生IV长度（默认16字节）
        :param iterations: 迭代次数（CryptoJS默认1）
        :param hash_func: 哈希函数（默认MD5）
        :return: 密钥和IV字节
        """
        derived = b''
        block = b''
    ...
    def _decrypt_cryptojs_aes(password: str, ciphertext_b64: str) -> Optional[bytes]:
        """
        解密 CryptoJS.AES.encrypt() 加密的数据，密文格式: base64("Salted__" + 8字节salt + ciphertext)
        :param password: 密码字符串
        :param ciphertext_b64: 密文字符串
        :return: 解密后的字节数据或None
        """
        try:
            # Base64 解码
    ...
    def decrypt_cookies(uuid: str, encrypted: str, password: str) -> Optional[Dict]:
        ...
        - 编码 = base64
        :param uuid: CookieCloud 账号 UUID
        :param encrypted: 密文字符串
        :param password: 密码字符串
        :return: 解密后的 Cookie 字典或None
        """
        try:
...
class CookieCloudFetcher:
    def __init__(self, server_url: str, uuid: str, password: str, timeout: int = 10):
        ...
        :param server_url: CookieCloud 服务器地址
        :param uuid: CookieCloud 账号 UUID
        :param password: CookieCloud 账号密码
...
class CookieValidator:
    ...
    def _parse_set_cookie(self, set_cookie_header: str) -> Dict[str, str]:
        """
        解析 Set-Cookie 响应头，提取 cookie name=value，支持多个 cookie（以逗号分隔的情况）
        ：param set_cookie_header: Set-Cookie 响应头
        :return: cookie name=value 字典
        """
        new_cookies = {}
    ...
    def _refresh_cookie(self, session: requests.Session) -> bool:
        """
        主动刷新 Cookie（使用 wr_rt 获取新的 wr_skey），参考 obsidian-weread-plugin 的 refreshCookie 实现
    ...
    def verify_cookie_validity(self) -> bool:
        """
        验证 Cookie 是否有效。通过调用 WeRead API 来验证 Cookie
